...
if(isset($_POST["type"]) && $_POST["type"]=='add')
{
	...
	if ($results) {
		$new_product = array(array('name'=>$obj->productName, 'code'=>$Product_ID , 'Pieces'=>$product_qty, 'unitPrice'=>$obj->Price));
		...
		if(isset($_SESSION["cart_session"]))
		{
			...
			foreach ($_SESSION["cart_session"] as $cart_itm)
			{
				if($cart_itm["code"] == $Product_ID){
					$product[] = array('name'=>$cart_itm["name"], 'code'=>$cart_itm["code"], 'Pieces'=>$product_qty, 'unitPrice'=>$cart_itm["unitPrice"]);
					$found = true;
				}else{
					$product[] = array('name'=>$cart_itm["name"], 'code'=>$cart_itm["code"], 'Pieces'=>$cart_itm["Pieces"], 'unitPrice'=>$cart_itm["unitPrice"]);
				}
			}
