...
<div class="inside-content">
    <div class="blank">
    	...
    	<div class="blankpage-main">
  		<?php $update=$_GET['update']; $result = mysqli_query($mysqli,"SELECT * FROM employee where Employee_ID ='$update'"); ?>
		<?php if($row = mysqli_fetch_array($result)) {?> 
		<div id="form_wrapper" class="form_wrapper">
		 <table>
			<form class="register active" action="EmployeeUpdate.php" method="POST" autocomplete="off">
   			<tr><td>
   			...
   			<tr><td>
				...
				<input type="text"class="form-control" id="name" name="name" value="<?php echo $row['Employee_Name'];?>"  placeholder="Full name" required>
			</td></tr>
   
   			<tr><td>   
				<label>Username:</label>
				<input type="text"class="form-control" id="username" name="username" value="<?php echo $row['Username'];?>" placeholder="User name" required>
		
			</td></tr>
   
   			<tr> <td>  
				<label>Password:</label>
				<input type="password"class="form-control" id="password" name="password" value="<?php echo $row['Password'];?>" placeholder="*******" required>
			...
			<td colspan="2">	
				...
				<button type="submit"  name="submit" value="Update" class="btn btn-warning btn-small" required="required"> <span class="a-btn-text">Update Admin</span>
				</button>
			</td>
			<div class="clear"></div>
			</div>
