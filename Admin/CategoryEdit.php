...
<?php $update=$_GET['update']; $result = mysqli_query($mysqli,"SELECT * FROM category where Category_ID ='$update'"); ?> <?php if($row = mysqli_fetch_array($result)) {?> 
  
	<div id="form_wrapper" class="form_wrapper">
		<table>
			<form class="register active" id="myForm" action="CategoryUpdate.php" method="POST">		
				<input type="hidden" id="ID" name="ID" value="<?php echo $row['Category_ID'];?>"  placeholder="ID" required>
			
			<tr><td>  
				<label>Category Name:</label>
				<input type="text"class="form-control" id="categoryName" name="categoryName"  value="<?php echo $row['Category_Name'];?>"  required>
			</td></tr>
   			
   			<tr><td>   
				<label>Description:</label>
				<input type="text"class="form-control" id="desp" name="description" value="<?php echo $row['Description'];?>" placeholder="Description" required>
			...
			<button type="submit"  name="submit" value="Update" class="btn btn-warning btn-small" required="required"> <span class="a-btn-text">Update Category</span>
			</button>
